...
import base64,io # type: ignore
from odoo import fields, models, api
from odoo.exceptions import UserError # type: ignore
from io import BytesIO
import pandas as pd
from . import metodos_prerequisites as metodos
from datetime import date
# Obtener la fecha actual
fecha_actual = date.today()

class DataPrerequisite(models.Model):
    _name = "dara_mallas.data_prerequisite"
    _description = "Datos de prerequisitos"
    ...
    period = fields.Char("Nombre del Periodo", readonly=True)


    file = fields.Binary("Archivo")    
    file_name = fields.Char("Nombre del Archivo")
    
    df_results=pd.DataFrame(columns=['period','subject_code','status','banner','odoo'])
    ...
    def create(self, values):
        ...
        if period_id:
            # Obtiene el nombre del período relacionado y lo asigna al campo 'period'.
            period_name = self.env['dara_mallas.period'].browse(period_id).name
            values['period'] = period_name  # Almacena el nombre del período en el campo 'period'.
        return super(DataPrerequisite, self).create(values)
    ...
    def load_or_prerequisitos(self):
        ...
        f = open('./source/insert-%s.txt'%(self.fecha),'w')
        delete = open('./source/delete-%s.txt'%(self.fecha),'w')
        #prerequisitos de homologaciones que cambiaron
        preHom = open('./source/insert-pre-hom-%s.txt'%(self.fecha),'w')
    ...
    def load_or_prerequisitos_homologation(self):
        ...
        SubInh = open('./source/update-subject-inherit-%s.txt'%(self.fecha),'w')
        for i,item_xls in siglas.iterrows():
            if item_xls['subject_code'] in res:
                id_prerequisite = metodos.get_id_prerequisito_odoo_head(item_xls['periodo'],item_xls['subject_code'])
                sql = """
    ...
    def execute(self):
        ...
        df = self.metodos.get_subject_prerequisito_odoo_per_period(period=self.period)
        ...
        self.df_results.to_csv('./source/resultados_validacion_scapreq-%s-%s.csv'%(self.fecha,period))
