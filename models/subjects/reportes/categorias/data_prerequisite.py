...
class DataPrerequisite(models.Model):
    ...
    def iniciar(self):
        ...
        self.metodos = metodos.Validator(database_banner=database_banner)
        self.df_results=pd.DataFrame(columns=['period','subject_code','status','banner','odoo'])
        #self.periodo = '202310'
        self.periodo = self.period_id.name
        self.fecha = fecha_actual.strftime("%d%m%Y")
        print("Entre a iniciar")
    ...
    def load_or_prerequisitos(self):
        ...
        #    'file_name':'prerequisitos-%s-%s.csv'%(fecha,self.period_id.name)          
        #    })
    ...
    def insert_or_prerequisitos_homologation(self,periodo,subject_code):
        ...
        print("Respusta insert_or sql ", sql)
        return sql
